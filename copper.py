...
with tab1:
    ...
    if submit_button and flag == 0:
        ...
        new_sample_encoder = item_loaded.transform(new_sample[:,7].astype(str).reshape(-1, 1))
        new_sample_be = status_loaded.transform(new_sample[:,8].astype(str).reshape(-1, 1))
        new_sample =np.concatenate((new_sample[:,:7],new_sample_encoder,new_sample_be),axis=1)
...
with tab2:
    ...
    if csubmit_button and cflag == 0:
        ...
        new_sample = np.array([[np.log(float(cquantity_tons)), np.log(float(cselling)), capplication,
                                ...
                                citem_type]])
        new_sample_encoder = ct_loaded.transform(new_sample[:, 8].astype(str).reshape(-1, 1))
        new_sample = np.concatenate((new_sample[:, [0, 1, 2, 3, 4, 5, 6, 7]], new_sample_encoder), axis=1)
